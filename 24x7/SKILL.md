...
| Claude exits 0 without a result | Task moves to failed/ with `OHNE ERGEBNIS` in log.md; a result is a file in output/ or a non-empty log.md. `CLAUDE_24X7_ERGEBNIS_PFLICHT=0` sends it to outbox/ instead, for tasks whose yield is meant to stay in the run log; the receipt still says `ergebnis: leer` either way |
...
- **Usage limit:** when a call exits non-zero with a rejected `rate_limit_event` in its stream-json (or, heuristically, a limit message as its error), the task goes back to `inbox/` instead of `failed/`, and the runner pauses until `resetsAt` (else `CLAUDE_24X7_LIMIT_PAUSE_SECONDS`, default 1800) while keeping the heartbeat alive. `CLAUDE_24X7_LIMIT_MAX_PAUSE_SECONDS` caps one pause, `CLAUDE_24X7_LIMIT_MAX_FOLGE` (default 12) caps consecutive pauses before failures count normally again.
