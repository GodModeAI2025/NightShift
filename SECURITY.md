...
- **It is off unless you switch it on.** `sandbox-exec -f nightshift-sandbox.sb ./nightshift-run.sh` is an optional line (README.md lines 237 and 367). Every other documented start path runs without it, `nightshift-run-bg.sh` and `runner-bg.sh` included, which `nohup bash` the runner directly.
- **Apple has deprecated `sandbox-exec`.** Its man page on macOS 27 says DEPRECATED in the first line. The single enforced boundary rests on a tool Apple has marked as going away.
- **The PreToolUse hook is not a boundary.** It carries `"matcher": "Bash"`, so the `Write` and `Edit` tools never reach it, and it greps the command string. These pass with exit 0: `R=rm; $R -rf /`, `find / -delete`, `git push --force origin main`, `cat ~/.ssh/id_rsa`. Treat it as a typo catcher.
...
1. **The PreToolUse hook fails open when `jq` is missing.** The hook reads `CMD=$(cat | jq -r ".tool_input.command // empty")`. Without `jq` the variable stays empty, `[ -n "$CMD" ]` is false, and the hook ends at `exit 0`. Nothing is blocked. With `jq` on PATH, `rm -rf /` gives exit 2 and "NIGHTSHIFT BLOCKED"; with `jq` removed from PATH the same input gives exit 0. The only trace is one `jq: command not found` on stderr from a hook that reports success, which nobody reads during an unattended night run. `jq` appears under prerequisites (README.md line 156) and is checked nowhere.
2. **The block pattern matches every `rm -rf`.** The pattern meant for `rm -rf *` arrives at `grep -E` as "rm -rf " followed by zero or more backslashes, and zero matches, so any continuation matches. `rm -rf node_modules` and `rm -rf dist` are blocked with exit 2. That is worse than a plain false positive: an overnight run stalls on a legitimate cleanup step, and the user's fix is to switch the hook off.
3. **A documented protection does not exist.** `nightshift/SKILL.md` line 171 and `24x7/SKILL.md` line 142 state the hook blocks fork bombs. There is no pattern for one; `:(){ :|:& };:` passes with exit 0.
...
8. **Autonomy zones are an agreement in the prompt.** The three zones sit as text in `RUNBOOK`. The red zone forbids files outside the project directory, touching secrets, and force push. `24x7/SKILL.md` line 141 goes further and claims "All work is confined to the workspace, no external paths allowed". Nothing enforces any of it: the hook sees Bash commands only, checks no paths, never sees `Write` or `Edit`, and lets force push through. What holds is the model's compliance plus, optionally, the seatbelt write restrictions.
