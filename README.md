...
24x7 runs the same check, with `CLAUDE_24X7_SANDBOXED` as the cross-check and `CLAUDE_24X7_ALLOW_UNSANDBOXED=1` as the opt-out. Since this release it also has a budget and a receipt, under `CLAUDE_24X7_BUDGET_USD` and `CLAUDE_24X7_BUDGET_TOKENS`. The budget covers the whole daemon run, not a single task, and the idle branch counts against it too, because idle calls Claude rather than sleeping. When the subscription's usage limit is reached, 24x7 puts the task back into `inbox/` and pauses until the reported reset instead of failing the whole queue within seconds; see `CLAUDE_24X7_LIMIT_PAUSE_SECONDS` in the 24x7 skill. A task reaches `outbox/` only when something came of it — a file in `output/` or a non-empty `log.md`; an exit code of 0 over an empty folder ends in `failed/`, and the receipt says `ergebnis: leer`. `CLAUDE_24X7_ERGEBNIS_PFLICHT=0` turns off the routing but not the finding: the task stays in `outbox/` and the receipt still says `leer`.
