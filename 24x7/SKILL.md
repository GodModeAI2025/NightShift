...
- **Usage limit:** when Claude reports its usage limit as the call's error, the task goes back to `inbox/` instead of `failed/`, and the runner pauses until the reported reset (else `CLAUDE_24X7_LIMIT_PAUSE_SECONDS`, default 1800) while keeping the heartbeat alive. `CLAUDE_24X7_LIMIT_MAX_PAUSE_SECONDS` caps one pause, `CLAUDE_24X7_LIMIT_MAX_FOLGE` (default 12) caps consecutive pauses before failures count normally again.
